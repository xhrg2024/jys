...
function EntityExplorePage({ navigate }) {
  ...
  const handleSelectEntity = async (entity) => {
    ...
    try {
      ...
      // 获取实体关系
      const relRes = await fetch(`/entity/${encodeURIComponent(entity.name)}/relations`);
      const relData = await relRes.json();
      setEntityRelations(relData);
    } catch (err) {
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {entityInfo ? (
            <div style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: 20 }}>
              <h3 style={{ fontSize: 18, fontWeight: 700, color: C.text, margin: "0 0 12px", fontFamily: "'Noto Serif SC', serif" }}>
                {selectedEntity?.name}
              </h3>
              <div style={{ fontSize: 13, color: C.textM, lineHeight: 1.8 }}>
                {entityInfo.info}
              </div>
            </div>
          ) : (
          ...
          {entityRelations && entityRelations.relations && (
            <div style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: 20 }}>
              <h4 style={{ fontSize: 15, fontWeight: 600, color: C.text, margin: "0 0 12px" }}>关联关系</h4>
